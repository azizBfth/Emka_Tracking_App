        ...
        if (dataMap.containsKey('events')) {
          ...
          for (var d in dataMap['events']) {
            ...
            switch (event.type.toString()) {
              case 'geofenceEnter':
                notifType = '';
                NotificationService().showNotification(event.id, deviceName,
                    "Entrée dans le Périmètre Virtuel", 10);
                return;
              ...
              case 'geofenceExit':
                ...
                NotificationService().showNotification(
                    event.id, deviceName, "Sortie du Périmètre Virtuel", 10);
                return;
              case 'deviceOverspeed':
                NotificationService().showNotification(
                    event.id, deviceName, "Vitesse Limite Dépassée", 10);
                return;
              case 'deviceFuelDrop':
                NotificationService().showNotification(
                    event.id, deviceName, "Perte de Carburant", 10);
                return;
              case 'maintenance':
                NotificationService().showNotification(
                    event.id, deviceName, "Maintenance Requis", 10);
                return;
...
*/
  ...
  Future<List<Event>> getEvents(
    ...
    List eventType = ["geofenceExit","geofenceEnter","deviceOverspeed","deviceFuelDrop","maintenance"];
    String cookie = await getCookie();
    ...
    if (response.statusCode == 200) {
      ...
      for (var ev in response.data) {
        if (Event.fromJson(ev).type != aux && eventType.contains(Event.fromJson(ev).type)
           ) {
  ...
  Future<List<Maintenances>> getAlerteMaintenancesById(
      {required id}) async {
    String cookie = await getCookie();
